...
void abpoa_set_graph_node(abpoa_graph_t *abg, int node_i) {
    ...
    abg->node[node_i].read_ids_n = 0;abg->node[node_i].array = NULL;
}

void abpoa_free_node(abpoa_node_t *node, int n) {
    int i, j;
    for (i = 0; i < n; ++i) {
        if(node[i].isok == 0) continue;
        ...
        if((i != 0) && (i != 1) && (node[i].array != NULL)){
            
            if (node[i].array->m > 0) {
                int j;
                
                for (j = 0; j < node[i].array->m; ++j) {
                    if (node[i].array->s[j].m){
                        free(node[i].array->s[j].s);
                    } 
                }
                free(node[i].array->s); 
                free(node[i].array->base);
                free(node[i].array->offset);
            }
            free(node[i].array);
        }
    }
...
int abpoa_add_graph_node(abpoa_graph_t *abg, uint8_t base) {
    ...
    // add node
    if(abg->node[0].isok != 1) abg->node[0].isok = 1;
    if(abg->node[1].isok != 1) abg->node[1].isok = 1;
    abg->node[node_id].base = base;
    abg->node[node_id].isok = 1;
    ++abg->node_n;
    return node_id;
}

seg_ment *seg_ment_init(void){
   seg_ment *s = (seg_ment*)_err_malloc(sizeof(seg_ment));
   s->n = s->m = 0;
   s->offset = 0;
   s->base = 0;
   s->s = 0;
...
seg_ment *seg_ment_realloc(seg_ment *r) {
    if (r->n >= r->m) {
        int m;
        if (r->m == 0) m = 1;
        else m = MAX_OF_TWO(r->n, (r->m) << 1);
        r->s = (kstring_t*)_err_realloc(r->s, m * sizeof(kstring_t));
        r->offset = (int*)_err_realloc(r->offset, m * sizeof(int));
        r->base = (uint8_t*)_err_realloc(r->base, m * sizeof(uint8_t));
        int i;
        for (i = r->m; i < m; ++i) {
            r->s[i] = (kstring_t){0,0,0};
            r->offset[i] = 0;
            r->base[i] = 0;
        }
        r->m = m;
...
int abpoa_add_graph_node1(abpoa_graph_t *abg, uint8_t base, kstring_t *seg_name, int i) {
    int node_id = abg->node_n;
    abpoa_realloc_graph_node(abg);
    // add node
    //fprintf(stderr,"seg_name:%s\n",seg_name->s);
    abg->node[node_id].array = seg_ment_init();
    seg_ment_realloc(abg->node[node_id].array);
    int n = abg->node[node_id].array->n;
    abg->node[node_id].array->base[n] = base;
    abg->node[node_id].array->offset[n] = i;
    kputsn(seg_name->s, seg_name->l, abg->node[node_id].array->s+n);
    abg->node[node_id].array->n++;
    if(abg->node[0].isok != 1) abg->node[0].isok = 1;
    if(abg->node[1].isok != 1) abg->node[1].isok = 1;
    abg->node[node_id].base = base;
    abg->node[node_id].isok = 1;
